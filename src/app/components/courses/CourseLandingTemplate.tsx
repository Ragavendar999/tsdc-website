...
export function CourseLandingTemplate({ course }: { course: CourseLandingData }) {
  ...
  return (
    <section className="site-section-bg relative overflow-hidden px-5 py-12 text-[#081225] md:px-8">
      <div className="mx-auto max-w-6xl rounded-[2.5rem] bg-white px-5 py-6 md:px-10 md:py-10">
        <motion.div
          initial={{ opacity: 0, y: 28 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7 }}
          className="relative overflow-hidden rounded-[2.4rem] px-6 py-8 text-white md:px-10 md:py-10"
          style={{ backgroundColor: course.deep }}
        >
          <div className="absolute -left-10 top-8 h-32 w-32 rounded-full bg-white/10" />
          <div className="absolute -right-8 bottom-8 h-36 w-36 rounded-[2rem] bg-white/12" />
            ...
            <div>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                {course.heroPoints.map((point) => (
                  <div key={point} className="flex items-start gap-2 rounded-2xl bg-white/12 px-4 py-3 text-sm font-bold">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-white px-7 py-3.5 text-sm font-black transition hover:scale-[1.02]"
                  style={{ color: course.deep }}
                >
                  Apply Now
                  <ArrowRight size={16} />
                </button>
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-white/45 px-7 py-3.5 text-sm font-black text-white transition hover:bg-white/12"
                >
                  <FileDown size={16} />
                  Download Syllabus
                </button>
            ...
            <motion.div whileHover={{ y: -8, rotate: 1 }} className="relative">
              <div className="overflow-hidden rounded-[2rem] bg-white p-3">
                <Image
                  ...
                  priority
                />
                <div className="absolute bottom-7 left-7 right-7 rounded-[1.4rem] bg-white/95 p-4 text-[#081225]">
        ...
        <div className="py-14 text-center">
          <p className="mx-auto mb-3 inline-flex rounded-full px-4 py-2 text-sm font-black" style={{ backgroundColor: course.soft, color: course.deep }}>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {course.outcomes.map((outcome, index) => (
            <motion.div
              ...
              className="rounded-[1.8rem] border border-[#e2e8f5] p-6"
              style={{ backgroundColor: index % 2 === 0 ? course.soft : '#f8fbff' }}
            >
              <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl text-sm font-black text-white" style={{ backgroundColor: course.accent }}>
        ...
        <div className="my-12 rounded-[2rem] border border-[#e2e8f5] bg-[#fffaf4] p-6 md:flex md:items-center md:justify-between md:gap-8">
          ...
          <button
            type="button"
            onClick={openSyllabusPopup}
            className="mt-5 inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-black text-white md:mt-0"
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {course.modules.map((module, index) => (
            <motion.div
              ...
              style={{ backgroundColor: index % 3 === 0 ? course.deep : index % 3 === 1 ? course.accent : '#4562b0' }}
            >
              <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-white/14" />
        ...
        <div className="my-12 grid gap-5 rounded-[2rem] border border-[#e2e8f5] bg-white p-6 md:grid-cols-3">
          <div className="rounded-[1.5rem] p-5" style={{ backgroundColor: course.soft }}>
          ...
          <div className="rounded-[1.5rem] bg-[#fff4f3] p-5">
        ...
        <div className="rounded-[2rem] p-7 text-white md:grid md:grid-cols-[1fr_0.8fr] md:items-center md:gap-8" style={{ backgroundColor: course.deep }}>
          <div>
          ...
          <div className="mt-6 rounded-[1.5rem] bg-white p-5 md:mt-0">
            <div className="space-y-3">
              <div className="rounded-full border border-[#e2e8f5] px-4 py-3 text-sm font-semibold text-[#667085]">Your name</div>
              <div className="rounded-full border border-[#e2e8f5] px-4 py-3 text-sm font-semibold text-[#667085]">Mobile number</div>
              <button
                type="button"
                onClick={() => openApplyPopup(`${course.popupInterest.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-bottom-cta`)}
                className="flex w-full items-center justify-center gap-2 rounded-full px-5 py-3.5 text-sm font-black text-white"
                style={{ backgroundColor: course.accent }}
              >
                Open Contact Form
                <ArrowRight size={16} />
